...
/**
 * Auralis — WebGL ambient background (layered simplex noise, glowing light,
 * film grain). Verbatim port of the shadcn/21st-dev `auralis` component,
 * adapted to plain JS/JSX: the `cn` helper call is replaced with template
 * join, and the empty overlay slot is dropped (callers overlay their own
 * content). No dependencies beyond React.
 */
...
export const AURALIS_DEFAULT_COLORS = ["#ef4444", "#dc2626", "#b91c1c"];

export default function Auralis({
  colors = AURALIS_DEFAULT_COLORS,
  speed = 0.3,
  grain = 0.6,
  height = "100vh",
  className = "",
  ...
  return (
    <div
      ref={containerRef}
      style={{ height, ...style }}
      className={`relative w-full overflow-hidden bg-[#010103] ${className}`}
    >
      <canvas
        ref={canvasRef}
