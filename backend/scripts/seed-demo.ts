 ...
 *   4. Campaign metadata is indexed into Postgres via POST /api/campaigns
...
// Pulls in hardhat-ethers' NetworkConnection type augmentation (connection.ethers).
...
async function authenticate(wallet: ReturnType<typeof ethers.Wallet.prototype.connect> | any, address: string) {
...
async function main() {
  ...
  console.log("3. Indexing campaign metadata into Postgres...");
  ...
  const aliceContract = contract.connect(aliceSigner) as typeof contract;
  const bobContract = contract.connect(bobSigner) as typeof contract;
