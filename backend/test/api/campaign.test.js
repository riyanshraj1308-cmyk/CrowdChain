...
const { Campaign, User, Milestone } = await import("../../src/models/index.js");
const { createApp } = await import("../../src/app.js");

const app = createApp();
const JWT_SECRET = "test-secret-key-please-change-in-prod";

function tokenFor(userId, walletAddress) {
  return jwt.sign({ userId, walletAddress }, JWT_SECRET, { expiresIn: "1h" });
}

// Minimal chainable cursor stand-in for .populate().sort().skip().limit().lean()
...
describe("Campaign API", () => {
  ...
  it("creates a campaign with a valid body and token", async () => {
    ...
    User.findOneAndUpdate.mockResolvedValue({ id: "user-1", walletAddress: wallet });
    Milestone.insertMany.mockResolvedValue([]);
    Campaign.create.mockResolvedValue({
      id: "campaign-1",
      title: "Solar-powered water pumps",
      toObject: () => ({ id: "campaign-1", title: "Solar-powered water pumps" }),
    });
  ...
  it("returns 409 when the contract campaign id is already indexed", async () => {
    ...
    Campaign.findOne.mockResolvedValue({ id: "existing" });
  ...
  it("blocks a non-creator from updating a campaign", async () => {
    ...
    // requireCampaignCreator uses findById(...).populate(...); emulate awaiting the chain
    Campaign.findById.mockReturnValue({
      populate: () =>
        Promise.resolve({
          id: "11111111-1111-1111-1111-111111111111",
          creatorId: { walletAddress: "0x1111111111111111111111111111111111111a" },
        }),
    });
  ...
  it("lists campaigns with pagination", async () => {
    Campaign.find.mockReturnValue(cursor([]));
    Campaign.countDocuments.mockResolvedValue(0);
