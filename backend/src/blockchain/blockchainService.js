...
export async function verifyTransactionEvent(txHash, eventName, minConfirmations = 1) {
...
export async function getOnChainCampaign(contractCampaignId) {
  const contract = getReadOnlyContract();
  return contract.getCampaign(contractCampaignId);
}

export async function getOnChainMilestone(contractCampaignId, milestoneId) {
  const contract = getReadOnlyContract();
